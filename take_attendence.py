...
def save_csv(info_dict):
    if f"{cur_date}.csv" in os.listdir("./csv_files"):
        with open(f"./csv_files/{cur_date}.csv","a") as csv_file:
            titles = ["Enrollment No","Name","Branch","Date","Day","Time"]
            writer = csv.DictWriter(csv_file,titles)
            writer.writerow(info_dict)
    else:
        with open (f"./csv_files/{cur_date}.csv","w") as csv_file:
            titles = ["Enrollment No","Name","Branch","Date","Day","Time"]
            writer = csv.DictWriter(csv_file,titles)
            writer.writeheader()
            writer.writerow(info_dict)
...
def load_encoded_file():
    ...
    with open('./database/enc_img_eno_list.p','rb') as file: 
        old_encoded_image,en_no_list = pickle.load(file=file)


def load_student_info():
    global student_dict
    with open('./database/student_info.json','r') as file: 
    ...
    if k == ord('p'):
        ...
        for encoded_face,face_loc in zip(new_encoded_images,roi_location):
            ...
            if matches[predicted_index] and face_distance[predicted_index]<0.5:
                play_music("./database/Correct_ans.mp3")
...
def take_image():
    face_capture = "./pre-model/haarcascade_frontalface_default.xml"
